...
static enum ich_chipset ifd2_platform_to_chipset(const int pindex)
{
	...
	case PLATFORM_ICL:
	case PLATFORM_CML: /* Comet Lake LP uses same PCH series as Ice Lake */
...
static void dump_fpsba(const struct fdbar *fdb, const struct fpsba *fpsba)
{
	...
	if (ifd_version >= IFD_VERSION_2) {
		...
		default:
			hap_strap = 0;
			hap_bit = 16;
			hap_strap_name = "PCHSTRP0";
			break;
		}
		printf("HAP bit is %sset (checked %s bit %d)\n",
		       fpsba->pchstrp[hap_strap] & (1 << hap_bit) ? "" : "not ",
		       hap_strap_name, hap_bit);
...
static void fpsba_set_altmedisable(struct fpsba *fpsba, struct fmsba *fmsba, bool altmedisable)
{
	if (ifd_version >= IFD_VERSION_2) {
		...
		printf("%sting the HAP bit (%s bit 16) to %s Intel ME...\n",
			altmedisable ? "Set" : "Unset",
			hap_strap_name,
			altmedisable ? "disable" : "enable");
		if (altmedisable)
			fpsba->pchstrp[hap_strap] |= (1 << 16);
		else
			fpsba->pchstrp[hap_strap] &= ~(1 << 16);
	} else {
		if (chipset >= CHIPSET_ICH8 && chipset <= CHIPSET_ICH10) {
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "S:V:df:F:D:C:M:xi:n:O:s:p:T:elrugEcvth?",
		...
		case 'p':
			...
			} else if (!strcmp(optarg, "cml")) {
				platform = PLATFORM_CML;
